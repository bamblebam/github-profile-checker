...
class FusionCharts:
    ...
    def render(self):
        ...
        self.readyJson = json.dumps(
            self.constructorOptions, ensure_ascii=False)

        if isinstance(self.timeSeriesObject, TimeSeries):
            self.readyJson = self.readyJson.replace(
                "__TS__", self.timeSeriesObject.GetDataSource())
        ...
        for key, value in self.eventOptions.items():
            self.readyJson = self.readyJson + \
                FusionCharts.eventTemplate.replace(
                    '__chartId__', self.constructorOptions['id'])
            self.readyJson = self.readyJson.replace(
                "_fceventname_", key).replace("_fceventbody_", value)

        # FusionCharts Render method will create chart
        self.readyJson = self.readyJson + \
            FusionCharts.renderTemplate.replace(
                '__chartId__', self.constructorOptions['id'])
        self.readyJson = FusionCharts.baseTemplate.replace(
            "__FC__", self.readyJson)

        if isinstance(self.timeSeriesObject, TimeSeries):
            self.readyJson = self.readyJson.replace(
                "__TS__", self.timeSeriesObject.GetDataStore())
        else:
...
class FusionTable:
    ...
    def Sort(self, columnName, columnOrderBy):
        ...
        sortedData = "sort([{0}])".format(
            "{{column: '{0}', order: '{1}'}}".format(columnName, orderby))
        self.stringBuilder.AppendLine(
            "fusionTable = fusionTable.query({0});".format(sortedData))
    ...
    def CreateFilter(self, filterType, columnName, *values):
        ...
        switcher = {
            ...
            FusionTable.FilterType.Between: "FusionCharts.DataStore.Operators.{0}('{1}', {2}, {3})".format(
                fx, columnName, my_list[0], my_list[1])
        }
    ...
    def ApplyFilter(self, filter):
        if len(filter) > 0:
            self.stringBuilder.AppendLine(
                "fusionTable = fusionTable.query(" + filter + ");")
